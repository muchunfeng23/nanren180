...
public class UserController {
	...
	public String login(HttpServletRequest request){
		...
		//判断是否存在openId,存在说明注册过，没存在说明需要注册
		boolean isExist = userService.isExist(openIdAndSessionKey.getOpenid());
		if(!isExist){
			String userInfoParam = request.getParameter("userInfo");
		...
		logger.info("openid = " + openIdAndSessionKey.getOpenid() + "  session_key = " + openIdAndSessionKey.getSession_key());
	...
	public String test(){
		logger.info("test...");
		return "ok" + userService.isExist("hello");
	}
